...
import { MasterService, Designation } from '../../../services/master.service';
...
@Component({
  ...
  template: `
    <div class="master-page">
      <div class="header">
        <h2>Designations</h2>
        <button class="btn-primary" (click)="openAddForm()">+ Add Designation</button>
      </div>

      <div class="modal" *ngIf="showForm" (click)="closeModalBackdrop($event)">
        <div class="modal-content" (click)="$event.stopPropagation()">
          <div class="modal-header">
            <h3>{{ isEditMode ? 'Edit Designation' : 'Add New Designation' }}</h3>
            ...
            <div class="form-body">
              <div class="form-grid">
                <div class="form-group">
                  <label>Title *</label>
                  <input type="text" [(ngModel)]="designation.title" name="title" required>
                </div>
                <div class="form-group">
                  <label>Code *</label>
                  <input type="text" [(ngModel)]="designation.code" name="code" required>
                </div>
                <div class="form-group">
                  <label>Level *</label>
                  <input type="number" [(ngModel)]="designation.level" name="level" required>
                </div>
                <div class="form-group full-width">
                  <label>Description</label>
                  <textarea [(ngModel)]="designation.description" name="description" rows="2"></textarea>
                </div>
                <div class="form-group checkbox-group">
                  <input type="checkbox" [(ngModel)]="designation.is_active" name="is_active" id="is_active">
      ...
      <div class="table-card">
        <table class="data-table" *ngIf="designations && designations.length > 0">
          <thead>
            <tr>
              <th>Code</th>
              <th>Title</th>
          ...
          <tbody>
            <tr *ngFor="let item of designations">
              <td>{{ item.code }}</td>
              <td>{{ item.title }}</td>
              <td>{{ item.level }}</td>
        ...
        <div *ngIf="!designations || designations.length === 0" class="no-data">
          <p>No designations found</p>
          <button class="btn-primary" (click)="openAddForm()">+ Add First Designation</button>
        </div>
      </div>
    </div>
  `,
  styles: [`
    ...
    .form-group input, .form-group textarea { padding: 12px; border: 2px solid #e0e0e0; border-radius: 8px; }
...
export class DesignationsComponent implements OnInit {
  ...
  designation: Designation = { title: '', code: '', description: '', level: 1, is_active: true };
  showForm = false;
  isEditMode = false;
  saving = false;
  ...
  load(): void {
    this.service.getDesignations().subscribe({
      next: (data) => this.designations = Array.isArray(data) ? data : (data as any).results || [],
      error: (e) => console.error(e)
    });
  }

  openAddForm(): void {
    this.designation = { title: '', code: '', description: '', level: 1, is_active: true };
  ...
  onSubmit(): void {
    ...
    const op = this.isEditMode
      ? this.service.updateDesignation(this.designation.id!, this.designation)
